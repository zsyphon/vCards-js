		...
		getFormattedString: function(vCard) {
			...
			if (vCard.cellPhone) {
				...
				vCard.cellPhone.forEach(
					function(number){
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;VALUE=uri;TYPE=cell:tel:' + e(number) + nl();
						} else {
			...
			if (vCard.homePhone) {
				...
				vCard.homePhone.forEach(
					function(number) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;VALUE=uri;TYPE=home:tel:' + e(number) + nl();
						} else {
			...
			if (vCard.workPhone) {
				...
				vCard.workPhone.forEach(
					function(number) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;VALUE=uri;TYPE=work:tel:' + e(number) + nl();
			...
			if (vCard.homeFax) {
				...
				vCard.homeFax.forEach(
					function(number) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;VALUE=uri;TYPE=fax:tel:' + e(number) + nl();
			...
			if (vCard.otherPhone) {
				...
				vCard.otherPhone.forEach(
					function(number) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;VALUE=uri;TYPE=other:tel:' + e(number) + nl();
